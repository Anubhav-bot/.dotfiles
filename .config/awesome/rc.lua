...
local function run_once(cmd_arr)
    for _, cmd in ipairs(cmd_arr) do
        local command = string.format("pgrep -u $USER -fx '%s' > /dev/null || %s", cmd, cmd)
...
globalkeys = mytable.join(
    ...
    -- Screen brightness
    awful.key({ }, "XF86MonBrightnessUp", function () os.execute("brightnessctl s 1%+") end,
              {description = "+1%", group = "hotkeys"}),
    awful.key({ }, "XF86MonBrightnessDown", function () os.execute("brightnessctl s 1%-") end,
              {description = "-1%", group = "hotkeys"}),
